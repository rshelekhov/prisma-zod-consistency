...
The skill packages (`skill-claude-code`, `skill-codex`) and `checks` are private/internal and do **not** get versioned via changesets — they ride with the repo's git tags. Only `@prisma-zod-consistency/cli` is published to npm and managed by changesets.
